...
13/06/2022  Sharon.li      ������������ģʽ�����̲�ѯע��״̬��example
=============================================================================*/
...
void ql_wifiscan_callback ( unsigned char scan_count, const char *scan_info)
{
	...
	jSonOutput = convert_to_json(scan_info);
    if (jSonOutput != NULL) {
        printf("%s", jSonOutput);
...
void ql_nw_test_wifiscan_task()
{
	...
	while(1)
	{
		...
		ret = ql_nw_get_wifiscan_info(15000,2,5,5,0,0);//��ʱʱ��15s,����10��
...
static void ql_test_cell_task(void)
{
	...
	while(1)
	{
        ...
        if(ql_nw_cell_info->lte_info_valid)
        {
            for(i=0;i<ql_nw_cell_info->lte_info_num;i++)
            {
				if(ql_nw_cell_info->lte_info[i].flag == 0)//����С��
				{
...
void ql_nw_set_test_task()
{
	...
	info.preferred_nw_mode = QL_NW_PREF_NET_TYPE_LTE;//������LTE��ʽ
	info.roaming_pref = 1;
	...
	{   
		...
		{
			ql_nw_get_drxtm(&drx);
			ql_nw_get_gdcnt(&sendbyte,&recvbyte);//����ͳ��
		}
...
//application_init(ql_nw_test_init, "ql_nw_test_init", 10, 0);
//application_init(ql_test_cell_task, "ql_nw_set_test_task", 10, 2);
