...
#define QL_WIFI_MSG_LOG(msg, ...)      ql_app_log(msg, ##__VA_ARGS__)
//#define QL_WIFI_MSG_LOG(msg, ...)        printf(msg, ##__VA_ARGS__)

#define QL_WIFI_MSG_TASK_EVENT_CNT 5
#define QL_WIFI_MSG_MSG_QUEUE_CNT  100
#define SWIFI_MSG_PRIORITY         81
        ...
        while (protocol_info.data_len > 0)
        {
            ...
            if (result == QL_WIFI_NPTO_PARSE_RESULT_SUCC)
            {
                ...
                case QL_WIFI_NPTO_802_3_STREAM: {
                    ...
                    {
						ql_wifi_msg_cmd_info_s info = 
						{
							.msg_id = QL_WIFI_NPTO_802_3_STREAM,
							.msg_info = msg_ptr
						};
				...
				//case QL_WIFI_NPTO_RESET_IND_CMD:
				{
                    ql_rtos_mutex_try_lock(ql_wifi_msg_mutex);
                    if (manager->net_notify_cb != NULL)
                    {
                        ql_wifi_msg_cmd_info_s info = 
						{
							.msg_id = protocol_head_info.command,
							.msg_info = protocol_head_info.data
						};
                ...
                default: {
                    QL_WIFI_MSG_LOG("command=%d error\r\n", protocol_head_info.command);
                    break;
                }
                }
            }
...
ql_wifi_msg_errcode_e ql_wifi_msg_init(void *argv)
{
    ...
    if (NULL == ql_wifi_msg_mutex)
    {
        if (QL_OSI_SUCCESS != ql_rtos_mutex_create(&(ql_wifi_msg_mutex)))
        {
			...
			QL_WIFI_MSG_LOG("ql_wifi_msg_init creat mutex err!!\r\n");
            return ret;
        }
    }
    ql_rtos_mutex_try_lock(ql_wifi_msg_mutex);
    ...
    memset(manager, 0, sizeof(ql_wifi_msg_manager_s));
    if (QL_OSI_SUCCESS != ql_rtos_task_create(&manager->task, 4096, SWIFI_MSG_PRIORITY, "ql_wifi_msg", ql_wifi_msg_thread, NULL, QL_WIFI_MSG_TASK_EVENT_CNT))
    {
