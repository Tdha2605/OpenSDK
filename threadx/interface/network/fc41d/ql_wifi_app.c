...
#define QL_WIFI_APP_LOG(msg, ...)      ql_app_log(msg, ##__VA_ARGS__)
//#define QL_WIFI_APP_LOG(msg, ...)    printf(msg, ##__VA_ARGS__)
...
typedef struct
{
	...
	char password[QL_WIFI_PASS_MAX_LEN];
...
ql_wifi_app_errcode_e ql_wifi_app_sta_enable(ql_wifi_app_sta_status_cb sta_status_cb)
{
    ...
    memcpy(&(start.password),ap_user_info->password,QL_WIFI_PASS_MAX_LEN);
    free(ap_user_info);
    ...
    if (manager->output_data_msg_cb)
    {
		...
		{
			.cmd = QL_WIFI_APP_STA_ENABLE_CMD,
			.data = (unsigned char *)&start, 
			.data_len = sizeof(ql_wifi_app_start_info_s)
		};
...
ql_wifi_app_errcode_e ql_wifi_app_sta_disable(void *argv)
{
	...
	if (manager->output_data_msg_cb)
	{
		ql_wifi_msg_output_data_info_s info = 
		{
			.cmd  = QL_WIFI_APP_STA_DISABLE_CMD, 
			.data = &data, 
			.data_len = 1
		};
...
#if 0
ql_wifi_app_errcode_e ql_wifi_app_spi_test(void *argv)
	{
	ql_wifi_app_manager_s *manager = &ql_wifi_app_manager;
	...
	ql_rtos_mutex_try_lock(ql_wifi_app_mutex);
	// Send wifi control command
	ql_wifi_msg_info_s info = 
	{
		.data = &data,
		.data_len = sizeof(data),
	};
	ql_wifi_spi_output_data_msg(&info);
	ql_rtos_mutex_unlock(ql_wifi_app_mutex);
	return QL_WIFI_APP_SUCCESS;
}
#endif
...
ql_wifi_app_errcode_e ql_wifi_app_init(void *argv)
{
    ...
    ret = ql_rtos_task_create(&manager->task, 4096, SWIFI_APP_PRIORITY, "ql_wapp",
