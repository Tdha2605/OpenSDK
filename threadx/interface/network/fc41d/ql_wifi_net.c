...
#define QL_WIFI_NET_LOG(msg, ...)        ql_app_log(msg, ##__VA_ARGS__)
//#define QL_WIFI_NET_LOG(msg, ...)          printf(msg, ##__VA_ARGS__)


#define QL_WIFI_NET_TASK_EVENT_CNT       5
#define QL_WIFI_NET_MSG_QUEUE_CNT        100

#define SWIFI_NET_PRIORITY           100
/*========================================================================
...
ql_wifi_net_errcode_e ql_wifi_net_init(void *argv)
{
	...
	if (QL_OSI_SUCCESS != ql_rtos_task_create(&(manager->task), 4096, SWIFI_NET_PRIORITY, "ql_wnet", ql_wifi_net_thread, NULL, QL_WIFI_NET_TASK_EVENT_CNT)){
