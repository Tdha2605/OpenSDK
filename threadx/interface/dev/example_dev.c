...
static void ql_dev_test_task(void * argv)
{
	...
	while(j<10000)
	{
		...
		//gavin.hu @20210109 support double IMEI
		ret = ql_dev_get_imei_by_simid(QL_DEV_SIM_0,data,sizeof(data));
		printf("ql_dev_get_imei_by_simid simid=0 ret=%d; IMEI:%s \n",ret,data);
		memset(data,0,sizeof(data));

		ret = ql_dev_get_imei_by_simid(QL_DEV_SIM_1,data,sizeof(data));
		printf("ql_dev_get_imei_by_simid simid=1 ret=%d; IMEI:%s \n",ret,data);
		memset(data,0,sizeof(data));
		...
		ret = ql_dev_get_modem_fun(&current_fun);
		printf("ql_dev_get_modem_fun ret=%d; modem_fun:%d \n",ret,current_fun);
...
/* print cpu_uid test*/
static void quec_cpu_uid_test(void * argv)
{
	ql_rtos_task_sleep_s(3);
    unsigned long long uid_ret=0;    

	ql_dev_get_cpu_uid( &uid_ret);

    printf("=========print cpu_uid test===========\r\n");
	printf("=========uid_ret= 0x%llx===========\r\n",uid_ret);
  
}
//application_init(quec_cpu_uid_test, "quec_cpu_uid_test", 2, 0);



static void ql_dev_test_get_mac(void * argv)
{
	unsigned int ret=-1;
	unsigned char mac_val[17] = {0};		//至少17个字�?
	printf("\n==========ql_dev_test_get_mac[start]==========\n");
	
	while(1)
	{
		ql_rtos_task_sleep_s(1);
		ret =ql_dev_get_mac(mac_val,sizeof(mac_val));
		printf("ret=%d; mac_val:%s \n",ret,mac_val);
	}
}

//application_init(ql_dev_test_get_mac, "ql_dev_test_get_mac", 2, 0);
