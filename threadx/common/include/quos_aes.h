...
#if defined(QUOS_AES256) && (1 == QUOS_AES256)
#define QUOS_AES_KEYLEN 32
#define QUOS_AES_keyExpSize 240
#elif defined(QUOS_AES192) && (1 ==  QUOS_AES192)
...
void Quos_aesInitCtx(Quos_AES_ctx_t *ctx, const char *key);
void Quos_aesInitCtxIv(Quos_AES_ctx_t *ctx, const char *key, const char *iv);
void Quos_aesCtxSetIv(Quos_AES_ctx_t *ctx, const char *iv);

/* buffer size MUST be mutile of QUOS_AES_BLOCKLEN; */
/* you need only Quos_aesInitCtx as IV is not used in ECB */
/* NB: ECB is considered insecure for most uses */
quint32_t Quos_aesEcbEncrypt(Quos_AES_ctx_t *ctx, const quint8_t *buf, quint32_t length);
void Quos_aesEcbDecrypt(Quos_AES_ctx_t *ctx, const quint8_t *buf, quint32_t length);

/* buffer size MUST be mutile of QUOS_AES_BLOCKLEN; */
/* Suggest https://en.wikipedia.org/wiki/Padding_(cryptography)#PKCS7 for padding scheme */
/* NOTES: you need to set IV in ctx via Quos_aesInitCtxIv() or Quos_aesCtxSetIv() */
/*        no IV should ever be reused with the same key */
quint32_t Quos_aesCbcEncrypt(Quos_AES_ctx_t *ctx, void *buf, quint32_t length);
void Quos_aesCbcDecrypt(Quos_AES_ctx_t *ctx, void *buf, quint32_t length);
...
/*        no IV should ever be reused with the same key */
void Quos_aesCtrXCrypt(Quos_AES_ctx_t *ctx, void *buf, quint32_t length);
