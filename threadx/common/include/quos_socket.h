...
typedef struct
{
    ...
    struct
    {
        socketSendIO_f sendx;
        socketCloseIO_f closex;
...
qbool Quos_socketCommonTx(const void *chlFd, void *peer, quint8_t sendCnt, quint32_t sendTimeout, socketsendNodeCb_f sendCB, socketRecvNodeCb_f recvCB,
                          quint32_t pkgId, quint8_t *buf, quint16_t bufLen, qbool priority);
#define Quos_socketTx(chlFd, peer, sendCnt, sendTimeout, sendCB, recvCB, pkgId, buf, bufLen) Quos_socketCommonTx(chlFd, peer, sendCnt, sendTimeout, sendCB, recvCB, pkgId, buf, bufLen, FALSE)
#define Quos_socketPriorityTx(chlFd, peer, sendCnt, sendTimeout, sendCB, recvCB, pkgId, buf, bufLen) Quos_socketCommonTx(chlFd, peer, sendCnt, sendTimeout, sendCB, recvCB, pkgId, buf, bufLen, TRUE)
qbool Quos_socketTxDisorder(const void *chlFd, void *peer, quint8_t *buf, quint16_t bufLen);
qbool Quos_socketTxAck(const void *chlFd, const void *peer, quint32_t pkgId, const void *recvData);
Quos_socketSendDataNode_t *Quos_socketGetOrderSendNode(const void *chlFd, quint32_t index);
qbool Quos_socketGetSockFdType(const void *chlFd, pointer_t *sockFd, quint8_t *type);
