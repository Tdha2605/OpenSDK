...
typedef struct
{
    enum
    {
        QIOT_SSLKEY_MODE_NONE,
        QIOT_SSLKEY_MODE_PSK,
        QIOT_SSLKEY_MODE_X509,
    } type;
    union
    {
        struct
...
pointer_t FUNCTION_ATTR_ROM Qhal_tcpClientInit(quint8_t *type, const char *ip, quint16_t r_port, quint32_t *connectTimeout);
pointer_t Qhal_udpInit(quint8_t *type,const char *ip, quint16_t r_port,void **peer);
pointer_t FUNCTION_ATTR_ROM Qhal_tcpSslClientInit(quint8_t *type, const char *hostname,  const char *ip, quint16_t r_port, quint32_t *connectTimeout);
pointer_t Qhal_udpSslInit(quint8_t *type, quint16_t l_port, const char *hostname,  const char *ip, quint16_t r_port);
