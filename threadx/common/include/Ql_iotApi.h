...
enum
{
    ...
    QIOT_ATECENT_TYPE_DATACOLL = 30,/* big data send value event */
};
enum
{
...
enum
{
    QIOT_DATA_COLL_SEND_SUCCESS = 0,        /* Data sent successfully . */
    QIOT_DATA_COLL_SEND_FAIL = 1,           /* Data sent failed. */
    QIOT_DATA_COLL_DELAYTIME = 2,           /* Data sent throttling (unit:second). */
    QIOT_DATA_COLL_SEND_RETRY = 3,          /* Data sent failed. Please try again. */
    QIOT_DATA_COLL_CONN_FAIL = 10,          /* Failed to connect to the data collection server. */
    QIOT_DATA_COLL_TRANS_FAIL = 11,         /* The data collection channel is terminated abnormally. */
    QIOT_DATA_COLL_DISENBLE = 12,           /* The data collection channel is not enabled.*/
    QIOT_DATA_COLL_CONN_RETRY = 13,         /* Server connection failed. Please try again.*/
    QIOT_DATA_COLL_CLOSE = 20,              /* The data collection channel has been closed. */
};
...
/**************************************************************************
 * 功能 @brief Request the NTP time and time zone
...
/**************************************************************************
* @brief Send data acquisition channel files.
* @param lastPack LastPack Whether the package is the last one.
* @param packNum  PackNum Sending number of subpackages.
* @param dataLen Date length(Unit:byte).
* @param data Data to report.
* @retval The result of send data.
* @remark The sending number of subpackages increases from 0. If there is only one data package, the data length needs to be less than 1024 bytes.
* If there are continuous packages, the first package needs to be 2 to the N power, and the length of subsequent packages must be the same as 
* that of the first package, and the length of the last package must be less than or equal to that of the first package.
***************************************************************************/
 ...
 * @retval True:Successful execution; False: Failed execution.
 * @remark The current authentication method of the device.
 ***************************************************************************/
qbool Ql_iotConfigGetAuth(quint32_t *authType);
#if (1 == QTH_ENABLE_ODOS)
/**************************************************************************
 * @brief Set the device authentication type to unique-DeviceSecret-per-device authentication.
 * @param [in] pk Devicekey, that is, customized unique identifier of the device. Maximum length: 16 bytes.
 * @param [in] ps ProductSecret generated when you create the product on Developer Center.
 * @param [in] dk Devicekey, that is, customized unique identifier of the device. Maximum length: 16 bytes.
 * @param [in] ds DeviceSecret generated after device authentication. The length is fixed to 32 bytes. Only when DK is customized, can DS be configured.
 * @retval True:Successful execution; False: Failed execution
 * @remark None of the parameters can be empty.
 ***************************************************************************/
